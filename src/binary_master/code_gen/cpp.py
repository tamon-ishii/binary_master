...
def cpp_type_of(field_type: Any) -> Tuple[str, Optional[str]]:
    ...
    if _safe_issubclass(field_type, RangeBase):
        cpp_name, _ = cpp_type_of(field_type._type)
        return cpp_name, f"Range: [{field_type._min}, {field_type._max}]"

    if _safe_issubclass(field_type, LengthOfBase):
        cpp_name, _ = cpp_type_of(field_type._type)
        return cpp_name, f"Length of '{field_type._target_field}'"

    if _safe_issubclass(field_type, CountOfBase):
        cpp_name, _ = cpp_type_of(field_type._type)
        return cpp_name, f"Count of '{field_type._target_field}'"
    ...
    if _safe_issubclass(field_type, MagicBase):
        ...
        if isinstance(expected, bytes):
            ...
            return fmt_map.get(fmt, "uint32_t"), f"Magic: {getattr(field_type, '_raw_val', '')!r}"
    ...
    if _safe_issubclass(field_type, ChecksumBase):
        ...
        return sz_map.get(sz, "uint32_t"), f"{getattr(field_type, '_algorithm', 'checksum').upper()} Checksum"
        ...
        if total_size is not None:
            try:
                ...
                if curr_size < total_size:
                    ...
                    lines.append(f"    std::array<uint8_t, {pad_len}> _padding; /**< Struct padding to total size {total_size} */")
            except Exception:
                pass
...
def generate_cpp_code(builder: Any) -> str:
    """Generate complete C++17 header from a Builder instance."""
...
def write_cpp(
    ...
    """Generate C++ code from a Builder and optionally save to file."""
