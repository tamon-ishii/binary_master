...
def go_type_of(field_type: Any) -> Tuple[str, Optional[str]]:
    ...
    if _safe_issubclass(field_type, MagicBase):
        ...
        if isinstance(expected, bytes):
            ...
            fmt_map = {"B": "uint8", "H": "uint16", "I": "uint32", "Q": "uint64"}
            return fmt_map.get(fmt, "uint32"), f"Magic: {getattr(field_type, '_raw_val', '')!r}"
    ...
    if _safe_issubclass(field_type, ChecksumBase):
        ...
        sz_map = {1: "uint8", 2: "uint16", 4: "uint32", 8: "uint64"}
        return sz_map.get(sz, "uint32"), f"{getattr(field_type, '_algorithm', 'checksum').upper()} Checksum"
...
def generate_go_code(builder: Any, package_name: str = "protocol") -> str:
    """Generate complete Go source file from a Builder instance."""
...
def write_go(
    ...
    """Generate Go code from a Builder and optionally save to file."""
