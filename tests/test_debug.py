...
def test_verify_success():
    """Test successful verification with verify and writer.verify."""
    from binary_master import verify

    w = BinaryWriter()
    w.write_uint32(0x12345678, name="magic")
    w.write_uint16(42, name="id")

    expected = b"\x78\x56\x34\x12\x2a\x00"
    assert w.verify(expected) is True
    assert verify(w, expected) is True
    assert verify(expected, w.to_bytes()) is True


def test_verify_failure_raises():
    """Test verify raises AssertionError with rich diff information on mismatch."""
    from binary_master import verify
    ...
    corrupted = b"\x78\x56\x34\x12\x99\x00"
    with pytest.raises(AssertionError) as excinfo:
        w.verify(corrupted)

    msg = str(excinfo.value)
    assert "Binary verification failed:" in msg
    assert "Differing byte count" in msg
    assert "id" in msg  # Field annotation present in diff


def test_verify_no_raise():
    """Test verify returns False when raise_error=False."""
    w = BinaryWriter()
    w.write_uint32(0x12345678)

    assert w.verify(b"\x00\x00\x00\x00", raise_error=False) is False
